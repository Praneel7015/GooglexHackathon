  ...
  return (
    <section className="max-w-[1280px] mx-auto px-4 md:px-8 py-8 md:py-12 grid md:grid-cols-[200px_1fr] gap-8 md:gap-12">
      <div>
        ...
        <div className="font-kn text-coffee/70 text-[13px] mt-0.5">ಎಸ್. ರಾಜೇಶ್ವರಿ</div>
        <hr className="border-line my-3" />
        <table className="font-sans text-[12px] w-full">
          <tbody>
            <tr><td className="text-[10px] uppercase tracking-wider text-coffee/55 pt-1">Role</td></tr>
            <tr><td className="pb-2">{officer.role}</td></tr>
            <tr><td className="text-[10px] uppercase tracking-wider text-coffee/55 pt-1">Ward</td></tr>
            <tr><td className="pb-2">{officer.ward} · {officer.wardName}</td></tr>
            <tr><td className="text-[10px] uppercase tracking-wider text-coffee/55 pt-1">Jurisdiction</td></tr>
            <tr><td className="pb-2">BBMP Roads · {officer.zone}</td></tr>
            <tr><td className="text-[10px] uppercase tracking-wider text-coffee/55 pt-1">Tenure</td></tr>
            <tr><td className="font-mono text-[11px]">since {officer.since}</td></tr>
          </tbody>
        </table>
      </div>

      <div>
        <Chip>Public record · Ward {officer.ward}</Chip>
        <h1 className="font-hand text-coffee text-3xl md:text-4xl mt-1 mb-5 tracking-tight">Officer Scorecard</h1>

        <div className="grid grid-cols-3 gap-3 mb-6">
          {[
            [officer.handled.toLocaleString(), 'complaints handled', 'last 12 months', 'coffee'],
            [`${officer.avgResponse}d`, 'avg response', 'under 5d benchmark', 'coffee'],
            [`${officer.resolutionRate}%`, 'resolution rate', 'top decile · all 198', 'olive']
          ].map(([n, l, s, c], i) => (
        ...
        {/* sparkline */}
        <Chip>12-month complaints handled</Chip>
        ...
        <Chip>Recently actioned · public log</Chip>
        <ul className="mt-2">
          {recent.map(r => (
        ...
        <Link to="/leaderboard" className="block mt-6 font-sans text-[12px] text-olive font-semibold underline">← back to all wards</Link>
      </div>
    </section>
  );
}
