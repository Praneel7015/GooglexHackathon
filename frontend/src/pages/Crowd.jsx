    ...
    <PhoneFrame>
      <div className="flex flex-col h-full p-4">
        <div className="text-center">
          <div className="text-[10px] uppercase tracking-wider font-sans text-coffee/55">A moment</div>
          <h1 className="font-hand text-[22px] text-coffee leading-tight mt-0.5">You're not alone here.</h1>
          <div className="font-kn text-[11px] text-coffee/65 mt-1">ನೀವು ಒಬ್ಬಂಟಿಯಲ್ಲ</div>
        </div>
        ...
        <Card tone="mist" padding="p-0" className="mt-3 h-72 overflow-hidden relative">
          <svg viewBox="0 0 220 280" width="100%" height="100%" preserveAspectRatio="xMidYMid slice">
            ...
            {Array.from({ length: bundleSize }).map((_, i) => {
              ...
              const delay = i * 30;
              return (
                <circle key={i} cx={x} cy={y} r="3" fill="#71816d" stroke="#342a21" strokeWidth=".7"
                  style={{ animation: `pop-in .35s ${delay}ms ease-out backwards` }}
                />
              );
            })}
        ...
        <Card tone="coffee" padding="px-3 py-2.5" className="mt-3">
          <div className="font-sans text-[9px] uppercase tracking-wider opacity-70">Bundled complaint</div>
          <div className="font-hand text-[16px] font-bold mt-0.5">Filed on behalf of {total} residents.</div>
          <div className="font-kn text-[10px] opacity-75 mt-0.5">{total} ನಿವಾಸಿಗಳ ಪರವಾಗಿ ಜಂಟಿ ದೂರು</div>
        </Card>

        <Button variant="primary" onClick={() => navigate('/track')} full className="mt-auto">Continue to tracker →</Button>
      </div>
    </PhoneFrame>
  );
}
